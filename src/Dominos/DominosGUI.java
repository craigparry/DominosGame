 ...
 * Player.java, GameState.java and MainGameLoop.java
 */
...
public class DominosGUI extends Application {
    ...
    /** Creates a tile to be used for the domino piece represented on the board
     *
     * @param num
     * @return Canvas
     */
    ...
    /**
     * toggles disable of the draw button
     * @param
     * @return void
     */
    ...
    /** updates the drawings for the pieces on the GUI gameboard
     * @param
     * @return void
     *
     */
    public void updateBoard() {
        ...
        blank.setBackground(new Background(new BackgroundFill(Color.BLUEVIOLET,
                        null, null)));
        botRow.getChildren().add(blank);
        for (Dominos d : game.getBoard()) {
            HBox temp = new HBox();
            temp.getChildren().addAll(drawDomino(d.getLeftSide()),
                                    drawDomino(d.getRightSide()));
        ...
        if (game.getGrave().isEmpty() && game.getComputer().existLegal(
                game.getBoard(),
                game.getComputer()) && !game.getHuman().existLegal(
                        game.getBoard(), game.getHuman())) {
            boolean compLegal = game.getComputer().computerMove(game.getBoard(),
                                            game.getComputer(), game.getGrave());
            if (compLegal) {
                game.setWinner("Computer");
            }
            updateBoard();
        ...
        if (game.getGrave().isEmpty() && !game.getHuman().existLegal(game.getBoard(),
                game.getHuman()) && !game.getComputer().existLegal(game.getBoard(),
                game.getComputer())) {
...
//                    "Information Dialog", "title");
    ...
    /**
     * adds dominos to the players gametray
     * @param domino
     * @return void
     */
    public void addGameTray(Dominos domino) {
        HBox temp = new HBox();
        temp.getChildren().addAll(drawDomino(domino.getLeftSide()),
                drawDomino(domino.getRightSide()));
...
//            System.out.println(domino.toString());
            ...
            if (right && left && !game.getBoard().isEmpty()) {
                ...
                leftButton.setOnAction(event3 -> {
                    ...
                    boolean legal;
                    legal = game.playTurnGUI(domino, game.getHuman(),
                            "left");
                    if (legal) {
                        playComputer(temp);
                    }
    ...
    public void start(Stage stage) {
        game.getHuman().setTray(game.getGrave().initDraw(
                game.getHuman().getTray()));
        game.getComputer().setTray(game.getGrave().initDraw(
                game.getComputer().getTray()));

        stage.setTitle("Domino Game");
        screen = new BorderPane();
        vbox = new VBox(1);
        vbox.setBackground(new Background(new BackgroundFill(Color.GREEN,
                null, null)));
        ...
        topRow.setBackground(new Background(new BackgroundFill(Color.BLUEVIOLET,
                null, null)));
        topRow.setBorder(new Border(new BorderStroke(Color.BLACK,
                BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));


        botRow = new HBox(1);

        botRow.setBackground(new Background(new BackgroundFill(Color.BLUEVIOLET,
                null, null)));
        botRow.setBorder(new Border(new BorderStroke(Color.BLACK,
                BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));
        ...
        screen.setBackground(new Background(new BackgroundFill(Color.BLACK,
                null, null)));
        gameTray.setBackground(new Background(new BackgroundFill(Color.BLUEVIOLET,
                null, null)));
        gameTray.setBorder(new Border(new BorderStroke(Color.BLACK,
                BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));
        gameTray.setPrefHeight(40);
        BorderPane.setAlignment(scrollTray, Pos.BOTTOM_CENTER);

        sidePanel = new VBox(5);
        sidePanel.setBackground(new Background(new BackgroundFill(Color.WHITE,
                null, null)));
        ...
        sidePanel.getChildren().addAll(resetButton, rightButton, leftButton,
                drawButton, empty);
